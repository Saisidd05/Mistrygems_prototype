  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Add Raw Material">
        <div className="grid grid-cols-2 gap-3"><input className="glass-input col-span-2" placeholder="Material name" value={newMaterial.name} onChange={event => setNewMaterial({ ...newMaterial, name: event.target.value })} /><input className="glass-input" placeholder="SKU" value={newMaterial.sku} onChange={event => setNewMaterial({ ...newMaterial, sku: event.target.value })} /><input className="glass-input" placeholder="Unit (kg, pcs)" value={newMaterial.unit} onChange={event => setNewMaterial({ ...newMaterial, unit: event.target.value })} /><input type="number" className="glass-input" placeholder="Opening stock" value={newMaterial.currentStock} onChange={event => setNewMaterial({ ...newMaterial, currentStock: Number(event.target.value) })} /><input type="number" className="glass-input" placeholder="Reorder level" value={newMaterial.reorderLevel} onChange={event => setNewMaterial({ ...newMaterial, reorderLevel: Number(event.target.value) })} /><input type="number" className="glass-input col-span-2" placeholder="Unit cost" value={newMaterial.unitCost} onChange={event => setNewMaterial({ ...newMaterial, unitCost: Number(event.target.value) })} /><GlowButton className="col-span-2" onClick={createMaterial}>Add Material</GlowButton></div>
      </Modal>
    </div>
  )
}
