...
export default function Inventory() {
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    const newItem: RawMaterial = {
      ...
      currentStock: 0,
      reorderLevel: newItemData.reorderLevel,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setAddItemModal(true)} className="btn-primary text-xs bg-gradient-to-r from-purple-600 to-pink-500">
      ...
      {addItemModal && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-md glass-card p-6 dark:bg-slate-900/90"
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-slate-900 dark:text-white">
                + Add New Item
              </h2>
              <button onClick={() => setAddItemModal(false)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
                <X className="w-4 h-4 text-slate-400" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddItem} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g., Steel Bar, Aluminum Sheet"
                  value={newItemData.name}
                  onChange={(e) => setNewItemData({...newItemData, name: e.target.value})}
                  required
                  className="w-full px-3 py-2.5 rounded-xl text-sm bg-slate-100/80 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/40 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-purple-500/40"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Unit Cost *</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    required
                    className="w-full px-3 py-2.5 rounded-xl text-sm bg-slate-100/80 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/40 text-slate-800 dark:text-slate-200 font-bold focus:outline-none"
                  />
                </div>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">Reorder Level</label>
                <input
                  type="number"
                  min="0"
                  placeholder="50"
                  value={newItemData.reorderLevel}
                  onChange={(e) => setNewItemData({...newItemData, reorderLevel: Number(e.target.value)})}
                  className="w-full px-3 py-2.5 rounded-xl text-sm bg-slate-100/80 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/40 text-slate-800 dark:text-slate-200 font-bold focus:outline-none"
                />
              </div>

              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setAddItemModal(false)} className="btn-secondary text-xs">Cancel</button>
