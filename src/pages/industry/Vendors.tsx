          ...
          {vendors.map(vendor => (
            <GlassCard key={vendor.id} className="p-5 flex flex-col gap-4">
              ...
              <div className="space-y-2 text-xs text-glass">
                <p className="flex items-start gap-2"><MapPin size={14} className="mt-0.5 text-[#00B4D8] flex-shrink-0" />{vendor.address || 'Address not available'}</p>
                <p className="flex items-center gap-2"><Mail size={14} className="text-[#00B4D8]" />{vendor.email || 'Email not available'}</p>
              </div>

              <div className="mt-auto flex items-center justify-between gap-3 pt-2 border-t border-glass/10">
                <span className="glass-badge text-[11px]"><Building2 size={12} className="inline mr-1" /> Registered</span>
                <GlowButton size="sm" variant="outline" onClick={() => setSelected(vendor)}>View Details</GlowButton>
              </div>
            </GlassCard>
          ))}
        </div>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div className="rounded-lg border border-glass/10 bg-white/5 p-3">
                ...
                <p className="mt-1 font-semibold text-highlight">{selected.ownerName || 'Workshop Owner'}</p>
              </div>
              <div className="rounded-lg border border-glass/10 bg-white/5 p-3">
                <p className="text-[10px] uppercase text-glass-dim">Email Address</p>
                <p className="mt-1 font-semibold text-highlight break-all">{selected.email || 'Not available'}</p>
              </div>
              <div className="rounded-lg border border-glass/10 bg-white/5 p-3">
                <p className="text-[10px] uppercase text-glass-dim">GSTIN</p>
                <p className="mt-1 font-semibold text-highlight">{selected.gstin || 'Not added'}</p>
              </div>
              <div className="rounded-lg border border-glass/10 bg-white/5 p-3">
                <p className="text-[10px] uppercase text-glass-dim">Registration Date</p>
                <p className="mt-1 flex items-center gap-2 font-semibold text-highlight"><CalendarDays size={14} />{formatDate(selected.createdAt)}</p>
              </div>
            </div>
            ...
            <div>
              ...
              {selected.reviews && selected.reviews.length > 0 ? (
                ...
                <GlassCard className="p-5 text-center text-glass-dim text-xs">No reviews yet for this workshop.</GlassCard>
              )}
            </div>
          </div>
        )}
