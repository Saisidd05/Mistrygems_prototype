...
function userFromRequest(req) {
  ...
  try {
    ...
    const industry = payload.accountType === 'industry' || String(payload.role || '').toLowerCase().includes('industry')
    return industry && typeof payload.id === 'string' ? { id: payload.id, companyId: String(payload.companyId || payload.id) } : null
  } catch { return null }
}
...
export default async function handler(req, res) {
  ...
  if (!user) return res.status(403).json({ error: 'Industry authorization is required.' })
  try {
    const collection = (await getIndustryDatabase()).collection(collectionName)
    const filter = { companyId: user.companyId }
    if (req.method === 'GET') return res.status(200).json(await collection.find(filter).sort({ createdAt: -1 }).toArray().then(rows => rows.map(({ _id, ...row }) => row)))
    if (req.method === 'POST') {
      ...
      const saved = { ...safeDocument, ownerId: user.id, companyId: user.companyId, createdAt: new Date().toISOString() }
